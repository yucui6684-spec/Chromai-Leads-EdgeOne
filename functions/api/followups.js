// 科诺美线索系统 API - followups（跟进记录 CRUD，Blob 存储）
...
async function updateMeta(store, fupsCount) {
  try {
    const meta = await store.get('meta', { type: 'json', consistency: 'strong' }) || {};
    meta.lastModified = Date.now();
    meta.fupsCount = fupsCount;
...
export async function onRequest({ request }) {
  ...
  if (request.method === 'GET') {
    try {
      const data = await store.get('followups', { type: 'json', consistency: 'strong' });
      if (data) {
  ...
  if (request.method === 'POST') {
    try {
      ...
      await store.set('followups', JSON.stringify(body));
